...
(setq org-directory "~/o/")
  ...
  (setq org-log-done 'time     ; Telling org to log DONE time
        org-ellipsis " ▾ ") ;  ▼  ; changing default ellipsis from ... to [...]
#+end_src
...
:MODIFIED: [2022-04-11 05:35]
:END:
Defines an action that needs to be done, but you cannot start the task at that moment itself because you’re missing some requirement that is needed to start this. An actionable item that needs to be "DONE"
***** \NEXT
:PROPERTIES:
...
:MODIFIED: [2022-04-11 05:35]
:END:
Marks tasks that you started, but then stopped for some reasons. They are not complete, but you decided that you don’t work on them anymore. Task was cancelled, aborted or is no longer applicable
***** \PROJ
:PROPERTIES:
